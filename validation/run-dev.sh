...
kubeconform -summary -strict \
  base/common/dev-namespace.yaml \
  base/common/dev-resourcequota.yaml \
  ...
  base/order-service/service.yaml \
  base/frontend/deployment.yaml \
  base/frontend/service.yaml \
  > validation-output/kubeconform-dev.txt

conftest test base --policy policy \
  > validation-output/conftest-dev.txt || true